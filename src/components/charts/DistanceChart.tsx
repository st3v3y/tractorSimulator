...
export const DistanceChart: React.FC<DistanceChartProps> = ({ gpsData }) => {
  ...
  useLayoutEffect(() => {
    ...
    if (hasTimestamps) {
      ...
      xAxis.get("renderer").labels.template.setAll({ fontSize: 10, paddingTop: 0, paddingBottom: 0, maxWidth: 60, oversizedBehavior: "truncate", dy: 10 });
    } 
    ...
    yAxis.get("renderer").labels.template.setAll({ fontSize: 10, paddingLeft: 0, paddingRight: 0, maxWidth: 40, oversizedBehavior: "truncate" });
    ...
    // Add smaller bullets for tooltip
    series.bullets.push(() =>
      am5.Bullet.new(root, {
        sprite: am5.Circle.new(root, {
          ...
          interactive: true,
          tooltipText:
            "[bold]Distance:[/] {valueY.formatNumber('#,##0.00')} km\n{valueX.formatDate('HH:mm:ss')}",
        }),
      })
    );
